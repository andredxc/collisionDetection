...
#define SDC_LOG "./sdcs-hardenedCD.log"
#define SPHERE_DATA "/home/andre/github/collisionDetection/hardened/sphereData.csv"
#define RECTANGLE_DATA "/home/andre/github/collisionDetection/hardened/rectangleData.csv"
SHAPE _shapeVector[NUMBER_OF_SHAPES];
int _shapeVectorIndex;
int _shapeVectorIndex2;
...
int main(int argc, char** argv){
    ...
    if(argc < 2 || strlen(argv[1]) == 0){
        // No file name given
        printf("Usage: ./collisionDetection <outputFile>, argc: %d\n", argc);
        printf("argv[0]: %s\n", argv[0]);
        printf("argv[1]: %s\n", argv[1]);
        printf("argv[2]: %s\n", argv[2]);
        return 0;
    ...
    loadShapes(SPHERE_DATA, RECTANGLE_DATA);
    collisionDetected = 1;
    while(collisionDetected){
        ...
        for(i = 0, i2 = 0; i < _shapeVectorIndex || i2 < _shapeVectorIndex || i < _shapeVectorIndex2 || i2 < _shapeVectorIndex2; i++, i2++){
            for(j = 0, j2 = 0; j < _shapeVectorIndex || j2 < _shapeVectorIndex || j < _shapeVectorIndex2 || j2 < _shapeVectorIndex2; j++, j2++){

                // Hardening
                if((_shapeVectorIndex != _shapeVectorIndex2) || (i != i2) || (j != j2)){
                    // SDC detected
                    logSDC();
                    printf("Terminated?\n");
                }
                ...
                if(!_shapeVector[i].object){
                    printf("NULL OBJECT AT i = %d (%d)\n", i, i2);
                    printShape(_shapeVector[i]);
                }
                else if(!_shapeVector[j].object){
                    printf("NULL OBJECT AT j = %d (%d)\n", j, j2);
...
void updatePositions(int elapsedTime_uSec){
    ...
    for(i = 0, i2 = 0; i < NUMBER_OF_SHAPES || i2 < NUMBER_OF_SHAPES; i++, i2++){
...
int loadShapes(char* sphereFileName, char* recFileName){
    ...
    // Hardening
    if(_shapeVectorIndex != _shapeVectorIndex2){
        logSDC();
    }
...
void logSDC(){
    ...
    if((file = fopen(SDC_LOG, "a"))){
        fprintf(file, "SDC detected\n");
        fclose(file);
    }
    printf("Terminating...\n");
    exit(0);
